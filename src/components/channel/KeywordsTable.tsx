...
function ScoreBar({ score }: { score: number }) {
  return (
    <div className="flex items-center gap-2 justify-center">
      <div
        className="h-1.5 w-14 rounded-full overflow-hidden flex-shrink-0"
        style={{ backgroundColor: "#E0D8CC" }}
      >
        <div
          className="h-full rounded-full transition-all"
          style={{ width: `${score}%`, backgroundColor: "#2C2C2C" }}
        />
      </div>
      <span className="text-xs font-bold w-7 text-right" style={{ color: "#2C2C2C" }}>
        {score}
      </span>
    </div>
  );
...
export function KeywordsTable({ keywords }: Props) {
  ...
  // Column definitions: label, key (null = not sortable), widthClass
  ...
  return (
    <div
      ...
      style={{ borderColor: "#D4CCB8", backgroundColor: "#FDFAF5" }}
    >
      <table className="w-full border-collapse" style={{ minWidth: "640px" }}>
        {/* colgroup for stable column widths */}
        <colgroup>
          ...
          {sorted.map((kw, idx) => {
            ...
            return (
              <>
                ...
                >
                  {/* # */}
                  <td className="px-3 py-3 text-center text-xs" style={{ color: "#8A8070" }}>
                    {idx + 1}
                  </td>
                  {/* Keyword */}
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isExpanded && (
                        <span className="text-[10px]" style={{ color: "#8A8070" }}>▲</span>
                      )}
                    </div>
                  </td>
                  {/* Usage */}
                  ...
                  {/* Score */}
                  <td className="px-3 py-3">
                    <ScoreBar score={kw.points} />
                  </td>
                  {/* Reason */}
                  <td className="px-3 py-3 text-xs" style={{ color: "#5A5A5A" }}>
                    <p>{kw.reason}</p>
                  </td>
                </tr>

                {/* Expanded row: related videos */}
                {isExpanded && (
                  <tr
                    key={`${kw.keyword}-exp`}
                    style={{ backgroundColor: "#EDE6D8", borderTop: "1px solid #D4CCB8" }}
                  >
                    <td colSpan={6} className="px-5 py-3">
                      <p className="mb-1.5 text-xs font-semibold" style={{ color: "#4A3C2A" }}>
                        関連動画タイトル
                      </p>
                        ...
                        {kw.videos.map((v, vi) => (
                          ...
                          >
                            <span
                              ...
                              style={{ backgroundColor: "#2C2C2C", color: "#F5F0E8" }}
                            >
                              {vi + 1}
                            </span>
                            {v}
